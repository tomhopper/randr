...
#' @description Returns a triangle-distributed vector within the \code{conf.level} tolerance interval defined by \code{lower} and \code{upper}.
...
#' @param lower (optional) numeric. The lower \code{(1 - conf.level)/2} tolerance limit.
#' @param upper (optional) numeric. The upper \code{(1 + conf.level)/2} tolerance limit.
#' @param conf.level (optional) numeric. The confidence level at which \code{upper} and \code{lower} are entered.
...
#' @return numeric vector with n elements randomly distributed so that approximately 99.7 percent of values will fall between lower and upper
#' @import VGAM
#' @examples
#' rtriangle_within(10)
#' rtriangle_within(10, 12, 10, 20)
#' summary(rtriangle_within(10000, 12, 10, 20))
rtriangle_within <- function(n, peak = 0.5, lower = 0, upper = 1, conf.level = 0.90, lower_bound = -Inf, upper_bound = Inf) {
  if (is.numeric(n) & is.numeric(lower) & is.numeric(upper) & is.numeric(conf.level) & is.numeric(peak) & is.numeric(lower) & is.numeric(upper)) {
    if (conf.level > 0 && conf.level < 1) {
      ...
      if(y_peak_l == y_peak_u) {
        ...
        d_p <- sqrt(((1-conf.level)*2)*y_peak_l*(upper - lower)/(sin(theta_l)/sin(alpha)+sin(theta_u)/sin(beta)))
...
#' @title Triangle distribution betwen given limits
...
rtriangle_between <- function(n, peak = 0.3, lower = 0, upper = 1) {
  if (is.numeric(n) & is.numeric(lower) & is.numeric(upper) & is.numeric(peak)) {
    if(peak < upper & peak > lower){
      x <- VGAM::rtriangle(n, theta = peak, lower = lower, upper = upper)
      return(x)
    } else {
      invisible(NaN)
      stop("peak must be between lower and upper.")
    }
  } else {
    invisible(NaN)
